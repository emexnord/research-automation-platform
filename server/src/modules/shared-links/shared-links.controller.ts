...
export class SharedLinksController {
  ...
  async createSharedLink(@Body(new ValidationPipe()) body: CreateSharedLinkDto, @Req() req) {
    const user = req.user;
  ...
  @ApiBody({ type: AccessSharedLinkDto, examples: {
    withPassword: {
  ...
  async accessSharedLink(@Param('linkId') linkId: string, @Body(new ValidationPipe()) body: AccessSharedLinkDto, @Res() res) {
